...
type Props = {
    isSoldOut: (n: string) => {}
    boughtNumbers: NumberType[]
}

export default function BuyNumbers({ isSoldOut, boughtNumbers }: Props) {
    ...
    const chooseNumber = (n: string) => {
      ...
      const total = totalSelected.reduce((a, b) => a + b.qty, 0) * 0.25;
      
      editGameContext(total, 'total');
    ...
    const removeNumber = (n: string) => {
      ...
      editGameContext((totalSelected.reduce((a, b) => a + b.qty, 0) * 0.25), 'total')
      editGameContext(totalSelected, "numbers")
    }
