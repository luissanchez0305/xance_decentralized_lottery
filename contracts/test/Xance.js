...
describe("Xance", function () {
    async function deployFixture() {
        ...
        const d = new Date('2023-07-29T22:00:00Z');
        ...
        const xance = await Xance.deploy(token.target, expireTime);
    ...
    describe("Claim", function () {
        it("Should buy tickets and refund", async function () {
            ...
            await xance.connect(owner).setFirstPrize(101);
            
            await xance.connect(account1).claim();
            expect(await xance.getSoldNumbersByAddress(1, account1.address)).to.equal(0);
        });
    });
